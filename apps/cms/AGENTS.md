...
Raveo-specific rules for the CMS app. Read the root `AGENTS.md` first, then this file.
For general Payload CMS patterns, refer to the existing rules in `.cursor/rules/`.

## Stack

- PayloadCMS 3.x inside Next.js 15
...
## Critical: Cloudflare Context Detection

`payload.config.ts` must detect three runtime environments and provide the correct D1/R2 bindings to adapters. This is the most non-obvious pattern in the project — do not simplify it.

```
┌─────────────────────────────────────────────────┐
│ CLI (payload migrate, payload generate:types)   │
│   → getCloudflareContextFromWrangler()          │
│   → remoteBindings: true → real D1              │
├─────────────────────────────────────────────────┤
│ Dev server (pnpm dev)                           │
│   → getCloudflareContextFromWrangler()          │
│   → local SQLite emulates D1                    │
│   → local filesystem emulates R2               │
├─────────────────────────────────────────────────┤
│ Production Worker                               │
...
Detection logic in `src/payload.config.ts`:
...
- **No sharp** — `crop: false`, `focalPoint: false` on every Media collection. Never add image transformations.
- **No filesystem** — no `fs`, no subprocesses, no Node.js-only modules
- **Single bundle** — `@opennextjs/cloudflare` builds everything into one Worker file

## Collections

Current collections: `Users`, `Media`, `Categories`, `Posts`, `Pages`

### Adding a New Collection

1. Create `src/collections/MyCollection.ts`
2. Import and add to `collections` array in `src/payload.config.ts`
3. Run `pnpm --filter @raveo/cms payload migrate:create` to generate migration
4. Add migration to `src/migrations/index.ts`
5. Run `pnpm --filter @raveo/cms generate:types` to update shared types

### Slug Field Pattern

All collections with public URLs use this slug generation hook:
...
NFD normalization removes diacritics — required for Czech/Slovak content.
...
access: {
  read: ({ req: { user } }) => {
    if (user) return true
    return { status: { equals: 'published' } } // WHERE clause — not a filter
  },
...
The `read` returning an object is a SQL WHERE clause, not a post-query filter. Unauthenticated users automatically see only published content.

## Globals

Current globals: `Navigation`, `SiteSettings`

Globals are singletons — one document per global, no slug, no list view.
Access pattern: `read: () => true`, `update: authenticated`.
...
```typescript
// Always use overrideAccess: false in custom route handlers
const posts = await payload.find({
  collection: 'posts',
...
Exception: seed scripts and migration utilities may intentionally bypass access control.

## Migration Workflow

```bash
# 1. Make schema changes in collection files
# 2. Generate migration
pnpm --filter @raveo/cms payload migrate:create

# 3. Add migration to src/migrations/index.ts (manual step — do not forget)

# 4. Run locally
pnpm --filter @raveo/cms payload migrate

# 5. Deploy to remote D1
pnpm --filter @raveo/cms deploy:database
```
...
```bash
pnpm deploy:cms
  ├─ cors:prod         → updates public/_headers for production CORS
  ├─ deploy:database   → runs migrations on remote D1 + PRAGMA optimize
  ├─ deploy:app        → opennextjs-cloudflare build + wrangler deploy
  └─ cors:dev          → restores public/_headers for dev CORS
```

CORS is handled via `public/_headers` — Cloudflare reads this as static response headers.
Do not add CORS logic to middleware or Payload config.
...
| `PAYLOAD_SERVER_URL` | Yes (prod) | Public CMS URL for media URLs |
| `CLOUDFLARE_ENV` | No | Wrangler environment override |
| `PAYLOAD_LOG_LEVEL` | No | Log level in production |

## Wrangler Config

`wrangler.jsonc` is committed with `YOUR_*` placeholders for open-source distribution.
Real binding IDs are set locally only — never commit actual IDs.

Bindings:

- `D1` — Cloudflare D1 database
- `R2` — Cloudflare R2 bucket for media
